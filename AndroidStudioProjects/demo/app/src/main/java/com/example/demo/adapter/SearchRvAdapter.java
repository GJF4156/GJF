...
public class SearchRvAdapter extends RecyclerView.Adapter<SearchRvAdapter.ViewHolder> {
    ...
    private List<SortsBean.NewslistBean> mDataBeanList;//保存数据源的列表
    private OnItemClickListener mListener;//初始化监听接口

    public SearchRvAdapter(Context context, List<SortsBean.NewslistBean> dataBeanList, OnItemClickListener listener) {
    ...
    public void onBindViewHolder(@NonNull SearchRvAdapter.ViewHolder holder, final int position) {
            holder.name.setText(mDataBeanList.get(position).getName());
            holder.sortDescription.setText(mDataBeanList.get(position).getExplain());
        switch (mDataBeanList.get(position).getType()) {
            case 0:
                //可回收垃圾
